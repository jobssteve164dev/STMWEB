...
DOT V1 已打通固件生成、验真、发布与烧录纵向切片，并形成模块化组合协议的第一版软件闭环：服务端生成带标准端口、UART 资源绑定和摘要的不可变组合图，Runner 与 CMake 消费同一份图；蓝牙升级被选择时才进入最终构建，仅 SWD 组合不会携带该升级桥。运动、电池、参数和遥测仍待从旧工程拆成独立能力积木，第二种真实硬件也尚未接入，因此当前不宣称完整模块化或跨硬件通用。
