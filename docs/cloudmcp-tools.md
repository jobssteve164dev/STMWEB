...
某个用户可以通过自己拥有的 CloudMCP，让受授权的智能体完成 STMWEB 中已经具备服务端执行条件的调试与编译工作。STMWEB 仍是设备、调试会话、Runner、构建与制品的业务事实源。该 CloudMCP 只是这个用户选择的客户端，必须使用这个用户授权的 STMWEB API 连接，并只负责 MCP 映射、连接治理和调用审计。STMWEB 不拥有、发现或调用它，其他用户也不需要知道它存在。
