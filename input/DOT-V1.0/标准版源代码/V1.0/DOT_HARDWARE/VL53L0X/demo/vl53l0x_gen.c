...
vu16 Distance_data;//保存测距数据
VL53L0X_Error vl53l0x_set_mode(VL53L0X_Dev_t *dev,u8 mode) //VL53L0X 测量模式配置  dev:设备I2C参数结构体   mode: 0:默认;1:高精度;2:长距离
{
...
//VL53L0X 单次距离测量函数  dev:设备I2C参数结构体  pdata:保存测量数据结构体
...
//启动普通测量  dev：设备I2C参数结构体  mode模式配置 0:默认;1:高精度;2:长距离
...
void vl53l0x_read(VL53L0X_Dev_t *dev)
{
	  ...
	  addr = dev->I2cDevAddr;//保存设备原I2C地址
		if(Status==VL53L0X_ERROR_NONE)
		{
		 ...
		 if(addr==0x54)Distance=Distance_data;
		// else if(addr==0x56)Distance2=Distance_data;
		}
}
