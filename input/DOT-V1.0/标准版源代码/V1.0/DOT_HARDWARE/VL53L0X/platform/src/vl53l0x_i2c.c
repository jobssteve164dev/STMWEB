...
void VL53L0X_i2c_init(void)  //VL53L0X I2C初始化
{
	...
	GPIO_SetBits(GPIOB,GPIO_Pin_13);//PB8 输出高	
	GPIO_SetBits(GPIOB,GPIO_Pin_14);//PB8 输出高	

}
void VL_IIC_Start(void)  //产生IIC起始信号
{
	VL_SDA_OUT();//sda线输出
	VL_IIC_SDA=1;	  	  
	VL_IIC_SCL=1;
...
void VL_IIC_Stop(void)  //产生IIC停止信号
{
	VL_SDA_OUT();//sda线输出
	VL_IIC_SCL=0;
...
void VL_IIC_NAck(void)  //不产生ACK应答	
{
	VL_IIC_SCL=0;
	VL_SDA_OUT();
	VL_IIC_SDA=1;
...
void VL_IIC_Send_Byte(u8 txd) //IIC发送一个字节  返回从机有无应答 1，有应答  0，无应答		
{                        
    u8 t;   
	  VL_SDA_OUT(); 	    
...
u8 VL_IIC_Read_Byte(unsigned char ack) //读1个字节，ack=1时，发送ACK，ack=0，发送nACK   
{
	...
	if (!ack)VL_IIC_NAck();//发送nACK
	else  VL_IIC_Ack(); //发送ACK   
	return receive;
}
u8 VL_IIC_Write_1Byte(u8 SlaveAddress, u8 REG_Address,u8 REG_data)  //IIC写一个字节数据
{
	VL_IIC_Start();
...
u8 VL_IIC_Read_1Byte(u8 SlaveAddress, u8 REG_Address,u8 *REG_data)  //IIC读一个字节数据
{
	VL_IIC_Start();
...
u8 VL_IIC_Write_nByte(u8 SlaveAddress, u8 REG_Address,u16 len, u8 *buf)  //IIC写n字节数据
{
	VL_IIC_Start();
	VL_IIC_Send_Byte(SlaveAddress);//发写命令
	if(VL_IIC_Wait_Ack()) 
...
u8 VL_IIC_Read_nByte(u8 SlaveAddress, u8 REG_Address,u16 len,u8 *buf) //IIC读n字节数据
{
	VL_IIC_Start();
	VL_IIC_Send_Byte(SlaveAddress);//发写命令
	if(VL_IIC_Wait_Ack()) 
	...
	while(len)
	{
		if(len==1)*buf = VL_IIC_Read_Byte(0);	
		else	*buf = VL_IIC_Read_Byte(1);
		buf++;
		len--;
	}
	VL_IIC_Stop();//释放总线
...
//VL53L0X 写多个数据 address:地址 index:偏移地址 pdata:数据指针 count:长度 最大65535
u8 VL53L0X_write_multi(u8 address, u8 index,u8 *pdata,u16 count)
{
	u8 status = STATUS_OK;
	if(VL_IIC_Write_nByte(address,index,count,pdata))status  = STATUS_FAIL;
	return status;
}
//VL53L0X 读多个数据  address:地址 index:偏移地址 pdata:数据指针 count:长度 最大65535
u8 VL53L0X_read_multi(u8 address,u8 index,u8 *pdata,u16 count)
{
	u8 status = STATUS_OK;
	if(VL_IIC_Read_nByte(address,index,count,pdata))status  = STATUS_FAIL;
	return status;
}
//VL53L0X 写1个数据(单字节) address:地址 index:偏移地址 data:数据(8位)
...
//VL53L0X 写1个数据(双字节) address:地址 index:偏移地址 data:数据(16位)
...
//VL53L0X 写1个数据(四字节)  address:地址  index:偏移地址 data:数据(32位)
...
//VL53L0X 读1个数据(单字节) address:地址 index:偏移地址 data:数据(8位)
...
//VL53L0X 读个数据(双字节)address:地址 index:偏移地址 data:数据(16位)
...
//VL53L0X 读1个数据(四字节) address:地址 index:偏移地址 data:数据(32位)
