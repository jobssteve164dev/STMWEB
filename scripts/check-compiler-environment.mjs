...
assert.doesNotMatch(packageVerifier, /test-firmware-compiler-image\.sh/);
...
assert.match(runner, /stmweb\/compiler:v0\.3\.12/);
assert.match(serverEnv, /stmweb\/compiler:v0\.3\.12/);
assert.match(compose, /stmweb\/compiler:v0\.3\.12/);
assert.match(envExample, /stmweb\/compiler:v0\.3\.12/);
...
assert.match(packageVerifier, /while chunk := stream\.read\(1024 \* 1024\)/);
assert.doesNotMatch(packageVerifier, /hashlib\.sha256\(package\.extractfile\([^\n]+\.read\(\)\)/);
